...
function photon_pion_decay(
    ...
    if i_ion == 1
        pion_photon_sum .= 1.0e-99
        energy_pion .= 1.0e-99MeV
    end
    ...
    for ID in 1:1 # 7
        ...
        count(emis_γ .> 1.0e-99) < 1 && continue
        ...
        for i in 1:n_photon_pion
            ...
            write(
                j_unit, # photon_pion_decay_grid.dat
                n_grid, i,
                ...
                log10(photon_flux / energy_γ)
            )    # 5 Log10[photons/(cm²⋅sec⋅MeV)]
