function iter_finalize(
    ...
    Γ_downstream[i_iter] = 1 + ∑P_downstream / ∑KEdensity_downstream
    ...
    n_avg = min(i_iter, 4)
    pₓ_esc_avg = mean(pₓ_esc_flux_upstream[1:n_avg])
    energy_esc_avg = mean(energy_esc_flux_upstream[1:n_avg])
    ...
    energy_esc_flux_upstream[i_iter] = max(energy_esc_flux_upstream[i_iter], 1.0e-99)
    energy_esc_avg = max(energy_esc_avg, 1.0e-99)
    pₓ_esc_flux_upstream[i_iter] = max(pₓ_esc_flux_upstream[i_iter], 1.0e-99)
    pₓ_esc_avg = max(pₓ_esc_avg, 1.0e-99)
    println(
        outfile,
        " Esc. en flux FEB/upstream  for i_iter = ", i_iter, ":   en esc = ",
        energy_esc_flux_upstream[i_iter], "   Avg. esc en  = ", energy_esc_avg
    )
    println(
        outfile,
        " Esc. pxx flux FEB/upstream for i_iter = ", i_iter, ":  pxx esc = ",
        pₓ_esc_flux_upstream[i_iter], "   Avg. esc pxx = ", pₓ_esc_avg
    )
    ...
    println(outfile, "   Adiab index for far upstream particles   = ", Γ_upstream)
    println(outfile, "   Adiab index for downstream PRP particles = ", Γ_downstream_esc)
