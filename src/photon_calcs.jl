...
function photon_calcs(
    ...
    p_pf_cgs_cr    = zeros(0:num_psd_mom_bins+1)
    ...
    for i in 1:n_shells,
        ...
        if aa ≥ 1 # Pion emission from nuclei
            ...
            p_pf_cgs_therm .= dNdp_therm_pvals[:,n,2]
            dNdp_pf_therm  .= dNdp_therm[:,n,2]

            # For pion decay, need plasma frame distribution, which is index
            # 2 in the third dimension (1 is shock frame, 3 is ISM frame)
            p_pf_cgs_cr .= exp10.(psd_mom_bounds)
            ...
            if (count(dNdp_pf_therm .> 1e-99) + count(dNdp_pf_cr .> 1e-99)) ≥ 1
            ...
            if count(d²Ndp_slice .> 1e-99) ≥ 1
                photon_IC(n, p_pf_cgs_cr, num_psd_mom_bins, cos_bounds, num_psd_θ_bins, d²Ndp_slice,
