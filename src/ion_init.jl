function ion_init()
    ...
    if E_pcut_hi_rmproton < energy_rel_pt
        ...
        p_pcut_hi = aa * mp_cgs * c_cgs * √((E_pcut_hi_rmproton + 1)^2 - 1)
    end
    ...
    # the shock structure
    global n_pts_use, i_grid_in, xweight_in, ptot_pf_in, pb_pf_in, x_PT_cm_in, globals... = init_pop(
        do_fast_push, inp_distr, i_ion, m,
        T₀_ion, energy_inj, inj_weight, n_pts_inj, ρ_N₀_ion, x_grid_start, rg₀, η_mfp,
        x_fast_stop_rg, β₀, γ₀, u₀, n_ions, aa_ion, zz_ion, T₀_electron, oblique,
    ...
    @info("Finished init_pop on",
          i_iter, i_ion,
          n_pts_use, i_grid_in, xweight_in, ptot_pf_in,
          pb_pf_in, x_PT_cm_in, pxx_flux, pxz_flux, energy_flux)

    # Assign the various particle properties to the population
    weight_new[1:n_pts_use]  .= xweight_in[1:n_pts_use]
    ptot_pf_new[1:n_pts_use] .= pt_pf_in[1:n_pts_use]
    ...
    global weight_running = xweight_in[1]
